...
SELECT 
    pg_size_pretty(pg_database_size('Data_Analysis'));

/* Check the Table size */
SELECT 
    pg_size_pretty(pg_relation_size('store'));
...
SELECT 
    product_name, 
    COUNT(order_id) AS No_of_Products 
FROM 
    store
GROUP BY 
    product_name
...
/* Where can we trim loses? In which products? */
SELECT 
...
/* Find average sales per sub-category. */
SELECT 
    Sub_Category, 
    ROUND((AVG(sales)::NUMERIC), 2) AS avg_sales
FROM 
...
/* Find average profit per sub-category. */
SELECT 
    ROUND((AVG(profit)::NUMERIC), 2) AS avg_profit, 
    Sub_Category
FROM 
...
/* Find those customers who registered during 2013-2016 */
SELECT 
...
WHERE 
    states = 'California' 
    AND postal_code = 90032;

/* Find Top 20 customers who benefited the store. */
SELECT 
    customer_name, 
    profit, 
    city, 
...
/* Which state(S) is the superstore most successful in? Least? */
SELECT 
    states, 
    ROUND(SUM(sales)::NUMERIC, 2) AS state_sales
FROM 
    store
GROUP BY 
    states
...
SELECT 
    ...
    ROUND((SUM(profit):: NUMERIC / (SUM(sales) - SUM(profit)):: NUMERIC) * 100, 2) AS profit_percentage
FROM 
    store
GROUP BY 
    EXTRACT(YEAR FROM order_date)
...
SELECT 
    ROUND(SUM(sales)::NUMERIC, 2) AS prod_sales, 
    category, 
    sub_category
FROM 
    store
...
SELECT 
    COUNT(returned_items) AS Returned_Items_Count
FROM 
    store
WHERE 
